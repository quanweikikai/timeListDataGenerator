...
def weighted_choice(choices, weights):
    ...
    for i,c in enumerate(choices):
        if upto + norm_weights[i] > r:
            return c,i
        upto += weights[i]
    assert False, "in weighted_choice"



class ArtificialDataGenerator(object):
    def __init__(self, pi, params_files):
        self.data_generators = [PPCCADataGenerator(params_file) for params_file in params_files]
        self.pi = pi
        self.z = []
    def generate(self, num_data):
        ...
        for data_i in range(num_data):
            ...
            self.data.append(data_generator.generate())
            self.z.append(z_n)
        return self.data
