...
class ClusteringErrorRateCalculator(object):
    def __init__(self, serial_results, correct_cluster_labels, numCluster):
        ...
        for cluster_i in range(numCluster):
            result_need_in_a_cluster =  np.array(serial_results[-1])[correct_data_index_in_cluster[cluster_i]]
            cluster_index.append(int(round(np.average(result_need_in_a_cluster))))
        self.correct_cluster_labels = [cluster_index[label] for label in correct_cluster_labels]
        self.numData = len(self.correct_cluster_labels)

        self.serial_error_rate = []
        for time_t, results in enumerate(serial_results):
            self.serial_error_rate.append(np.count_nonzero(results - self.correct_cluster_labels))
    ...
    def compareSomeOtherResult(self, other_results):
        ...
        for cluster_i in range(self.numCluster):
            result_need_in_a_cluster =  np.array(other_results)[correct_data_index_in_cluster[cluster_i]]
            cluster_index.append(int(round(np.average(result_need_in_a_cluster))))
        temp_correct_cluster_labels = [cluster_index[label] for label in self.correct_cluster_labels]
