...
def SplitEmbeddedVec(frames_labels, frames_embeddedVec):
    ...
    for frame_i in range(len(frames_labels)):
        labels_frames_embeddedVec[frames_labels[frame_i]].append(frames_embeddedVec[frame_i])
    return labels_frames_embeddedVec

def generateEmbeddedVec(frames_featureVec, embeddedTime, delayTime, numDeleteHeadFrames = 0, numSamplingFrames=1):
    frames_featureVec = np.array(frames_featureVec)
    frames_embeddedVector = []

    for frame_i in range(embeddedTime + delayTime, len(frames_featureVec)):
        tempEmbeddedVector = []
        for embeddedTime_i in range(0, embeddedTime, numSamplingFrames):
            tempEmbeddedVector.append(frames_featureVec[frame_i - embeddedTime_i - delayTime])
...
class MotionCaptureDataLoader(object):
    ...
    def Load(self):
        ...
        self.frames_positions = np.array(np.delete(temp, 0, 1))#TODO もっと綺麗にかけるはず

        self.convertToRelativePosition()

#        frames_speed = np.diff(self.frames_positions, axis=0)
#        self.frames_featureVec = np.hstack([self.frames_positions[1:], frames_speed]) #正規化したほうがいいかも？
        from sklearn.preprocessing import normalize
#        self.frames_featureVec = normalize(self.frames_positions[1:], axis=1)
        import sklearn.decomposition

        pca = sklearn.decomposition.PCA(n_components=0.9)  #if 0 < n_components < 1, means contribution
        self.frames_featureVec = pca.fit_transform(normalize(self.frames_positions[1:], axis=1))
    ...
    def convertToRelativePosition(self):
        ...
        frames_centerPos = copy.deepcopy(self.frames_positions[:,3*3:3*3+3]) #centerが3番目 × xyz
        for frame_i,positions in enumerate(self.frames_positions):
            for position_i,j in enumerate(positions):
                self.frames_positions[frame_i][position_i] -= frames_centerPos[frame_i][position_i % 3]

    def adjustNumFrames(self, numFrames):
        self.frames_featureVec = np.delete(self.frames_featureVec, np.s_[numFrames:],0)

    def generateEmbeddedVec(self, embeddedTime, delayTime, numDeleteHeadFrames = 0, numSamplingFrames = 1):
        frames_embeddedVector = generateEmbeddedVec(self.frames_featureVec, embeddedTime, delayTime, numDeleteHeadFrames, numSamplingFrames)
        return frames_embeddedVector
...
#    from PPCCAEMalgorithmn import PCCAEMalgorithmn
    ...
    frames_embeddedVecXt   = loaderX.generateEmbeddedVec(           1,         0, numDeleteHeadFrames = embeddedTime)


    data = [[np.matrix(embeddedVecXtkm).T, np.matrix(embeddedVecYtkm).T, np.matrix(embeddedVecXt).T] for i,[embeddedVecXtkm, embeddedVecYtkm, embeddedVecXt] in enumerate(izip(frames_embeddedVecXtkm, frames_embeddedVecYtkm, frames_embeddedVecXt))]
    ...
    pickle_dic = {"data_for_movie_x":loaderX.GenerateLabeledSplitedDataForMovie(labels_frames, embeddedTime, numSamplingFrames),
    ...
    filename = "/data1/keisuke.kawano/results/" + (datafileX + datafileY).replace(".trc","_").replace("./data/","")  +"%dclusters_%dembeddedTime_%ddelayTime_%dnumSumpingFrames_"%(numCluster, embeddedTime, delayTime, numSamplingFrames) + str(time.strftime("%Y%b%d_%a_%H%M%S", time.strptime(time.ctime())))+".txt"
